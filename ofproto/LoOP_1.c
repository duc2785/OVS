...
void get_data(const char *file_path, float array[][2]){
    FILE *fp;
    int count = 0;
    fp = fopen(file_path, "r");
        while (count < 1000 && fscanf(fp, "%f,%f", &array[count][0], &array[count][1]) == 2) {
        // printf("flow = %f, traffic = %f", array[count][0], array[count][1]);
        count++;
