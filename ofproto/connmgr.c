...
{
    char *buf;
    buf = malloc(9*sizeof(char));  // set 8 bytes to allocate for null terminator to make buf array -> String
    if (f == 1)
    {
        buf = "1.000000";
        return buf;
...
void connmgr_send_anomaly_detection(struct connmgr *mgr)
{
    ...
    LIST_FOR_EACH (ofconn, connmgr_node, &mgr->conns)
    {
        ...
        //Hieu//
        char * p_value1_1 = convert(value1_1);
        ofpbuf_put(msg,p_value1_1,n);

        //original//
        // ofpbuf_put(msg,convert(value1_1),n);

        ofconn_send(ofconn, msg, NULL);
        free(p_value1_1);  //important
    }
    gettimeofday(&end_s,NULL);
    // fp2 = fopen("/home/server2/Cuong_1sw_loop/6sw_test/result/time_send.log","a+");
    // fprintf(fp2,"%lu\n",(end_s.tv_sec - start_s.tv_sec)*1000000+end_s.tv_usec-start_s.tv_usec);
    // fclose(fp2);
    // fclose(fp1);
}
